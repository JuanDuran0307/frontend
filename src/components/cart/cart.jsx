...
import { ItemCart } from "../productCart/productsCart";
...
const Cart = () => {
  ...
  useEffect(() => {
    setProductsLength(
      cartItems?.reduce((previous, current) => previous + current.stock, 0)
    );
  }, [cartItems]);

  /* Obtenemos el precio total */
  const total = cartItems?.reduce(
    (previous, current) => previous + current.stock * current.precio,
    0
  );
